...
#include <LSM303.h>               //GS pimoroni sensor seems to work with this library
#include <ArduinoJson.h>          //https://github.com/bblanchon/ArduinoJson
#include <Wire.h>
#include <Adafruit_Sensor.h>
//#include <Adafruit_TSL2561_U.h>  //GS old light sensor
#include <Adafruit_TSL2591.h>      //GS new light sensor
...
byte tsl2461I2CAddr=0x29;      // replace with i2c used by the TSL259112Q  
...
// GS my substitute below
Adafruit_TSL2591 tsl = Adafruit_TSL2591(2591);  //Define the luminosity sensor. Parameter is an identifier
//Adafruit_HMC5883_Unified mag = Adafruit_HMC5883_Unified(12345);  //Define the magnetometer.  Also intializes LSM303.
LSM303 mag;   //GS Pimoroni magnetometer instead of adafruit.  
...
//The second restart it comes up with the correct IP address
  ...
  if (checkI2CAddr(tsl2461I2CAddr)) {
    if (!tsl.begin())
      ...
      tsl.setGain(TSL2591_GAIN_HIGH);            /* replace above with paramaters that work on TSL2591. MED is 25X */
      tsl.setTiming(TSL2591_INTEGRATIONTIME_100MS);      /* replace above with parameters that work on TSL2591. 100ms is shortest we can go */
     }
  }
  else {
    tsl2561Exists = false;
    flowState = STATE_NOT_WORKING;
    Serial.println("Did not find TSL2591, State changed to STATE_NOT_WORKING"); //GS made error msg better
  }
  ...
  if (hcm5883Exists) {
    if (!mag.init())
    {
      ...
      hcm5883Exists = false;
      flowState = STATE_NOT_WORKING;
      Serial.println("Magnetometer not found, State changed to STATE_NOT_WORKING");  //GS made error verbose
    }
    else {
...
void loop()
{
  if (hcm5883Exists & tsl2561Exists) {  //Only do this processing if the sensors exist.  Note the hcm5883 driver hangs if you attempt to access it and the hw doesnt exist
    //LSM303 event;
    //sensors_event_t event;     // doing a light sensor read using adafruit unified sensor api
    uint16_t lightread = tsl.getLuminosity(TSL2591_VISIBLE);  //GS replacing unified sensor api
    mag.read();     //GS read LSM3030
    // Serial.println(mag.m.y);    //for debugging
    float diffXSq = (previousXMagField - mag.m.x) * (previousXMagField - mag.m.x);
    float diffYSq = (previousYMagField - mag.m.y) * (previousYMagField - mag.m.y);
    float diffZSq = (previousZMagField - mag.m.z) * (previousZMagField - mag.m.z);
    // GS Does not seem to be a need to square these values, the magnetometer is sensitive enough without it.
    previousXMagField = mag.m.x;
    previousYMagField = mag.m.y;
    previousZMagField = mag.m.z;
    ...
    //tsl.getEvent(&event);
    //Serial.println(event.light);   //GS debug print out light level
    if (lightread > 0) {
  ...
  if (writeData) {
    ...
    //Serial.print("X: "); Serial.print(mag.m.x); Serial.print(",  Y: ");  Serial.print(mag.m.y); Serial.print(",  Z: ");  Serial.print(mag.m.z); Serial.print(",  Diff: ");  Serial.println(avgMagDiff);
  }
